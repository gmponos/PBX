...
$pages = selected_page($_SERVER['SCRIPT_NAME']);
	...
	if (!$_POST) { 
 	//	echo "NO post: ". "sdate: ". $_SESSION['sdate']. "edate:" . $_SESSION['edate'] . "filter:" . $_SESSION['filter']. "fval:" .$_SESSION['fval'] . " ". $_SESSION['Incoming'] . " " . $_SESSION['Outgoing'];
		...
		$sql = "SELECT calldate,src,dst,channel,split_part(split_part(dstchannel,'-',1),'/',2) as dstchan,billsec,disposition,direction,userfield,accountcode,uniqueid,recording,mcn FROM cdr WHERE (calldate between '$sqlsdate' and '$sqledate') $disposql $sqlfilter and direction in ( ";
		$totalsql = "SELECT count(*) AS total FROM cdr WHERE $filter_for_call_center (calldate between '$sqlsdate' and '$sqledate') $sqlfilter $disposql and direction in ( ";
		  if ($sql_inject != ''){
		...
		if(($result = @postg_query($sql, $db)) == 0)
			  ...
			  echo $sql;
                ...
                while( $row = pg_fetch_assoc($result))
                 	{ if ($_SESSION['access_recordings'] == 't') {
			 ...
			 $test = array(calldate=>$daytime,src=>$source,dst=>$destination,billsec=>$formated_bill,disposition=>$mydispo,accountcode=>$row[accountcode],rec=>$recfile,userfield=>$row[userfield] . ' ' . $network);
			 $data[] = $test;
		 	}
