...
Gr0wl.Base = Class.create({
	...
	create: function(styles) {
	    this.image.setStyle('position:absolute;display:none;z-index:9999');
		Element.setOpacity(this.image, 0.0);
		this.block = new Element('div');
        this.block.setStyle('position:absolute;display:none;z-index:10000;color:#fff;font: 12px/14px "Lucida Grande", Arial, Helvetica, Verdana, sans-serif;'+styles.div);
    ...
    show: function(options) {
        ...
        var fxOpts = {duration:0.9, from:0.0, sync:true};
        new Effect.Event({
            beforeStart: function() {
                new Effect.Parallel([
                    new Effect.Opacity(elements[0], Object.extend(fxOpts, {to:0.97})),
                    new Effect.Opacity(elements[1], Object.extend(fxOpts, {to:0.97}))
	...
	hide: function(elements) {
        ...
        new Effect.Parallel([
            ...
            new Effect.Opacity(elements[1], Object.extend(fxOpts, {from:0.97}))
        ], {
            duration:0.8, 
            queue: {position: 'end', scope: 'growlscope'},
...
Gr0wl.Smoke = Class.create(Gr0wl.Base, {
	...
	create: function($super, oArgs) {
		$super({
			div: 'width:298px;height:116px;',
			img: 'float:left;margin:12px;height:100px;',
	...
	show: function($super, options) {
        ...
        winBottom = document.body.getHeight();
		scrollTop = this.getScrollTop();
		delta = scrollTop+10+(last*120);
        delta = winBottom-10-(last*120);
