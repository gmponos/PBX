...
Proto.Menu = Class.create({
    initialize: function() {
        ...
        $('container').insert(this.container.insert(list).observe('contextmenu', Event.stop));
        if (this.ie) { $('container').insert(this.shim) }
    ...
    show: function(e) {
        ...
        var x = Event.pointer(e).x,
            ...
            vpOff = $('container').cumulativeScrollOffset(),
            ...
            elOff = {
                ...
                top: ((y - vpOff.top + elDim.height) > vpDim.height && (y - ( vpOff.top * -1 ) ) > elDim.height 
                    ? (y + (vpOff.top * -1) - elDim.height) : y + (vpOff.top * 1)) + 'px'
            };
