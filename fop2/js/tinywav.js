...
function TinyWav(pid, trigger) {
	...
	this.SoundState = function (state, position) {
		...
		this.State = state;
        debug(this.icon+" is in state "+state);
        if(state == "PLAYING") {
            debug("esta play, le pongo icono pause");
            $(''+this.icon).removeClassName('playicon');
            $(''+this.icon).addClassName('pauseicon');
        } else if(state=="STOPPED") {
            debug("esta stop, le pongo icono play");
            $(''+this.icon).addClassName('playicon');
            $(''+this.icon).removeClassName('pauseicon');
        } else if(state=="PAUSED") {
            debug("esta paused, le pongo icono play");
            $(''+this.icon).addClassName('playicon');
            $(''+this.icon).removeClassName('pauseicon');
        }
	}
